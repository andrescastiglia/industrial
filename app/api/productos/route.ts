...
export async function POST(request: NextRequest) {
  return withTrace("POST /api/productos", async (span) => {
    try {
      // Autenticar usuario
      const auth = authenticateApiRequest(request);
      ...
      `,
        ...
        span?.setAttribute("producto.created_id", nuevoProducto.producto_id);
        span?.setAttribute("producto.nombre", nuevoProducto.nombre_modelo);
        span?.setAttribute("producto.componentes_count", componentes.length);
