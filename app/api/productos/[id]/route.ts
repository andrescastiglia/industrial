...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { nombre_modelo, descripcion, ancho, alto, color, tipo_accionamiento, componentes = [] } = body
    ...
    try {
      ...
      const result = await client.query(
        ...
        RETURNING *
      `,
        [nombre_modelo, descripcion, ancho, alto, color, tipo_accionamiento, params.id],
      )
      ...
      await client.query("DELETE FROM Componentes_Producto WHERE producto_id = $1", [params.id])
      ...
      for (const componente of componentes) {
        ...
        `,
          [params.id, componente.materia_prima_id, componente.cantidad_necesaria, componente.angulo_corte],
        )
      }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      // Eliminar producto
      const result = await client.query("DELETE FROM Productos WHERE producto_id = $1 RETURNING *", [params.id])
