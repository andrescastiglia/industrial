...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await client.query("SELECT * FROM Proveedores WHERE proveedor_id = $1", [params.id])
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await client.query("DELETE FROM Proveedores WHERE proveedor_id = $1 RETURNING *", [params.id])
